...
const { filenames, nImages } = mapping;
...
console.log(
  "Category | Pair | " +
    embTypes.map((e) => `${e.key} min`).join(" | ") +
    " | " +
    embTypes.map((e) => `${e.key} avg`).join(" | "),
);
console.log(
  `---|---|${embTypes.map(() => "---").join("|")}|${embTypes.map(() => "---").join("|")}`,
);
...
for (const tp of testPairs) {
  ...
  console.log(
    `${tp.category} | ${gA.name} × ${gB.name} | ${mins.join(" | ")} | ${avgs.join(" | ")}`,
  );
}
...
function intraGroupMedianDist(
  emb: { data: Float32Array; dim: number },
  groupImages: string[],
): number {
...
function crossGroupOverlapFraction(
  emb: { data: Float32Array; dim: number },
  gA: string[],
  gB: string[],
  threshold: number,
...
console.log(
  "Category | Pair | intraA | intraB | crossMin | crossAvg | overlap@medA | overlap@medB | overlap@0.05 | overlap@0.10",
);
console.log("---|---|---|---|---|---|---|---|---|---");
...
for (const tp of testPairs) {
  ...
  const overlap010 = crossGroupOverlapFraction(colorEmb, gA.images, gB.images, 0.1);
