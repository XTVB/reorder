...
export function ComparePanel() {
  ...
  const closeCompare = useClusterStore((s) => s.closeCompare);
  const toggleCompareCandidate = useClusterStore((s) => s.toggleCompareCandidate);
  const setIncludeConfirmedGroups = useClusterStore((s) => s.setCompareIncludeConfirmedGroups);
  const addClusterToCompare = useClusterStore((s) => s.addClusterToCompare);
  const confirmMerge = useClusterStore((s) => s.confirmMerge);
  ...
  const visibleCandidates = filterCandidates(compare, clusterData.clusters, splitChildren);
      ...
      <div className="compare-header">
        ...
        <div className="compare-header-actions">
          <button
            ...
            disabled={compare.checked.size === 0}
          >
            merge selected ({compare.checked.size})
          </button>
...
function WinnerChoiceModal() {
  const pending = useClusterStore((s) => s.compare?.pendingWinnerChoice);
  const groups = useGroupStore((s) => s.groups);
  const cancelWinnerChoice = useClusterStore((s) => s.cancelWinnerChoice);
  const commitMergeWithWinner = useClusterStore((s) => s.commitMergeWithWinner);
...
function filterCandidates(
  ...
  splitChildren: ReturnType<typeof useClusterStore.getState>["splitChildren"],
): ClusterResultData[] {
