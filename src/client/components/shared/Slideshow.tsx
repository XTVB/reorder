...
import { type SlideshowTransition, useSessionStore } from "../../stores/core/sessionStore.ts";
...
export function Slideshow({
  ...
  const playing = useSessionStore((s) => s.slideshow.playing);
  const intervalMs = useSessionStore((s) => s.slideshow.intervalMs);
  const shuffle = useSessionStore((s) => s.slideshow.shuffle);
  const transition = useSessionStore((s) => s.slideshow.transition);
  const setPlaying = useSessionStore((s) => s.setSlideshowPlaying);
  const setInterval_ = useSessionStore((s) => s.setSlideshowInterval);
  const setShuffle = useSessionStore((s) => s.setSlideshowShuffle);
  const setTransition = useSessionStore((s) => s.setSlideshowTransition);
