...
export const useImageStore = create<ImageState>((set, get) => ({
  ...
  setImages: (images) => {
    ...
    const needsMapRebuild = images.length !== existingMap.size || images.some((i) => !existingMap.has(i.filename));
    const imageMap = needsMapRebuild ? new Map(images.map((i) => [i.filename, i])) : existingMap;
    const hasChanges = images.length > 0 && (images.length !== originalOrder.length || images.some((img, i) => img.filename !== originalOrder[i]));
